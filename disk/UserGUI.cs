...
public class UserGUI : MonoBehaviour
{
    ...
    private void OnGUI()
    {
        ...
        if (start && GUI.Button(new Rect(Screen.width / 2 - 175, Screen.height / 2 - 45, 130, 90), "kinematics mode"))
        {
            start = false;
            action.set_state(GameState.ROUND_START);
            action.startMode(true);
        }
        if (start && GUI.Button(new Rect(Screen.width / 2 - 25, Screen.height / 2 - 45, 130, 90), "physics mode"))
        {
            start = false;
            action.set_state(GameState.ROUND_START);
            action.startMode(false);
        }
